...
export function readLayoutFromFile(): Record<string, unknown> | null {
	...
	try {
		...
		console.error('[Arcadia] Failed to read layout file:', err);
		return null;
	}
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
	...
	try {
		...
		console.error('[Arcadia] Failed to write layout file:', err);
	}
}
...
export function migrateAndLoadLayout(
	...
	if (fromFile) {
		console.log('[Arcadia] Layout loaded from file');
		return fromFile;
	}
	...
	if (fromState) {
		console.log('[Arcadia] Migrating layout from workspace state to file');
	...
	if (defaultLayout) {
		console.log('[Arcadia] Writing bundled default layout to file');
...
/**
 * Watch ~/.arcadia/layout.json for external changes (other VS Code windows).
...
export function watchLayoutFile(
	...
	function checkForChange(): void {
		...
		try {
			...
			console.log('[Arcadia] External layout change detected');
			onExternalChange(layout);
		} catch (err) {
			console.error('[Arcadia] Error checking layout file:', err);
		}
	}
